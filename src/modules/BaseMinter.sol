...
abstract contract BaseMinter is IMinterModule {
    ...
    function setEditionMintPaused(
        ...
        uint128 mintId,
        bool paused
    ) public virtual onlyEditionOwner(edition) {
     ...
     */
    modifier onlyEditionOwner(address edition) virtual {
        if (
            ...
            if (affiliated) {
                ...
                affiliateFee = uint128(
                    (uint256(remainingPayment) *
                        uint256(baseData.affiliateFeeBPS)) / uint256(_MAX_BPS)
                );
